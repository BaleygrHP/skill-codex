...
- [dispatching-parallel-agents](skills/dispatching-parallel-agents/SKILL.md): Coordinate tasks, tickets, and Flows with independent scopes, explicit dependencies, and file ownership. The coordinator handles discovery, classification, dispatch, and aggregation; a separate coding executor implements and tests. Independent review uses a separate agent from the coordinator/explorer model family, following task risk and project requirements. Ordinary findings return to the executor; only difficult decisions or unresolved blockers go to the architect.
